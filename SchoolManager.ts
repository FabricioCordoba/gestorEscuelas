...
export class SchoolManager {
  ...
  expelStudent(
    ...
    if (deleteStudent >= 0) {
      ...
      console.log(
        `El alumno ${student.name} ${student.lastName} fue expulsado`
      );
    } else {
      console.log(
        `El alumno  ${student.name} ${student.lastName} no fue encontrado en el listado`
      );
    }
  ...
  fireTeacher(
    ...
    if (deleteTeacher >= 0) {
      ...
      console.log(
        `El docente ${teacher.name} ${teacher.lastName} fue despedido`
      );
    } else {
      console.log(
        `El docente ${teacher.name} ${teacher.lastName} no fue encontrado en el listado`
      );
    }
  ...
  editPerson(
    ...
    let personTeacher = listTeacher.findIndex(
      (listTeacher: { id: number }) => listTeacher.id === id
    );
    if (personStudent >= 0) {
      ...
      switch (dateEdit) {
        case "numPhone":
          student.numPhone = readLineSync
            .question("modifique numero de telefono del alumno:-->")
            .toLowerCase();
          break;
        case "address":
          student.address = readLineSync
            .question("modifique la direccion del alumno:-->")
            .toLowerCase();
          break;
        case "email":
          student.email = readLineSync
            .question("modifique el e-mail del alumno:-->")
            .toLowerCase();
          break;
          default:
            console.log("No se realizo ninguna modificacion");
      }
      ...
      switch (dateEdit) {
        case "numPhone":
          teacher.numPhone = readLineSync
            .question("modifique numero de telefono del docente:-->")
            .toLowerCase();
          break;
        case "address":
          teacher.address = readLineSync
            .question("modifique la direccion del docente:-->")
            .toLowerCase();
          break;
        case "email":
          teacher.email = readLineSync
            .question("modifique el e-mail del docente:-->")
            .toLowerCase();
          break;
          default:
            console.log("No se realizo ninguna modificacion");
  ...
  consultLists(
    search = readLineSync
      .question("ingrese el listado que desea ver: Alumnos o Profesores:-->")
      .toLowerCase()
  ) {
    if (search === "alumnos") {
      let listStudent = this.dataStudents();
  ...
  consultPerson(
    id = Number(
      readLineSync.question("Ingrese el DNI de la persona que busca:-->")
    )
  ) {
    let listStudent = this.dataStudents();
    let listTeacher = this.dataTeachers();
  ...
  listTeacherByStudent(
    id = Number(readLineSync.question("Ingrese DNI del Alumno:--> "))
  ) {
    let listStudents = this.dataStudents();
    let personStudent = listStudents.find(
      (listStudents: { id: number }) => listStudents.id === id
    );

    let teachers: string[] = personStudent.subjects.map(
      (subject: { teacher: string }) => subject.teacher
    );
    console.log(`Los Profesores de   ${personStudent.name}  son ${teachers}`);
  }
  listStudentByTeacher(
    id = Number(readLineSync.question("ingrese el DNI del Docente:--> "))
  ) {
    let listStudents = this.dataStudents();
    let listTeachers = this.dataTeachers();
    let personTeacher = listTeachers.find(
      (listTeacher: { id: number }) => listTeacher.id === id
    );
    let subjectTeacher: string = personTeacher.subject;

    let listTeacherStudents = listStudents.filter(
      (student: { subjects: { name: string }[] }) => {
    ...
    for (let i = 0; i < listTeacherStudents.length; i++) {
      students.push(" "+listTeacherStudents[i].name +" "+ listTeacherStudents[i].lastName);
     
    }

    console.log(`Los alumnos del docente  ${personTeacher.name} ${personTeacher.lastName}
      son: 
      ${students}`
    );
  }
  ...
  averageStudents() {
    this.dataStudents().forEach((persona: any) => {
      console.log(
        `Nombre: ${persona.name},  Apellido:${persona.lastName},  Promedio: ${persona.average}`
      );
  ...
  selectMenu() {
    ...
    console.log(
      "Por favor, marque la opcion deseada en el listado, o bien marque 0 (CANCEL) si desea salir del sistema"
    );
    let options = [
      "Inscribir Alumno",
      "Contratar Docente",
      "Expulsar Alumno",
      "Despedir Docente",
      "Editar Dato Alumno/Docente",
      "Consultar Lista Alumno/Docente",
      "Consultar dato de Persona",
      "Listado de Docente con sus Alumnos",
      "Listado de Alumno con sus Docentes",
      "Listado de Alumnos con Promedios",
    ];
    let optionSelect = readLineSync.keyInSelect(
      options,
      "Marque la opcion deseada en el listado"
    );
    ...
    while (optionSelect + 1 !== 0) {
      switch (option) {
        case "Inscribir Alumno":
          console.log(
            "opcion inscribir un Alumno, Ingrese los datos que se piden debajo"
          );
          this.setEnrollStudent();
          break;
        case "Contratar Docente":
          console.log(
            "opcion contratar un docente, Ingrese los datos que se piden debajo"
          );
          this.hireTeacher();
          break;
      ...
      optionSelect = readLineSync.keyInSelect(
        options,
        "Marque otra opcion para operar: "
      );
